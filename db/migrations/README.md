...
| `0047_dream_image` | `athena_dream.image_path` / `image_model`: the picture painted from each dream, stored in the private `athena-dreams` bucket (30-day lifecycle) and served through `GET /api/v1/dreams/:uuid/image`. |
| `0046_athena_dreams` | The Dreams log: `athena_dream` (one row per night), `athena_dream_step` (every statement she ran in `athena_mind`, verbatim, cascade-deleted with its night after 30 days) and `athena_dream_question` (what she wants to ask people; answers are kept as source data). Her own tables live in the separate `athena_mind` database, created by [`../mind-setup.js`](../mind-setup.js), not by a migration. See [`docs/architecture/dreaming.md`](../../../docs/architecture/dreaming.md). |
| `0045_memory_extraction_log` | One counts-only row per memory extraction (chat path and nightly sweep): what the model proposed, what was written, and why the rest was dropped (duplicate, low confidence, locked slot, malformed, over cap). Read by the nightly self-review. Extraction works without it. |
