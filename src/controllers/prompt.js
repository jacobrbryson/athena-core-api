...
function buildGuardianPersona(guardian) {
  const name =
    typeof guardian?.displayName === "string"
...
function buildIndexNudge(mission) {
  ...
  if (mission.transition === "code_accepted" && mission.latestEntry) {
    ...
    lines.push(
      `Just recovered — "${e.title}" (found by ${e.foundBy}):\n"""\n${e.record}\n"""`,
    );
  ...
  if (
    Array.isArray(mission.newConvergences) &&
    mission.newConvergences.length
  ) {
    ...
    if (Number.isFinite(reported) && Number.isFinite(total)) {
      lines.push(
        `So far ${reported} of ${total} families have reported in.${
          Array.isArray(mission.reporting.pending) &&
          mission.reporting.pending.length
...
`;
  ...
  if (game.status === "finished") {
    lines.push(
      "The game has ENDED this turn. Play the ending properly and warmly.",
    );
  }
  if (game.events.length) {
    lines.push(
      `**What just happened (already resolved — narrate exactly this):**`,
    );
...
`;
  ...
  if (guardian && decodes && !onboarding)
    prompt += buildDecodeAwareness(decodes);
  ...
  if (onboarding) {
    ...
    if (guardian && mission)
      prompt += buildPostWelcomeMissionNudge(mission, onboarding);
  }
  return prompt;
}
...
async function firstNameForProfile(profileId) {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT full_name FROM profile WHERE id = ? LIMIT 1;`,
      [profileId],
    );
...
const STRATEGIES = {
  ...
  companion: async (session, topics, options = {}) => {
    ...
    if (options.audience === "adult" && !options.guardian) {
      ...
      return buildAdultCompanionPrompt(session, memory, {
        ...options,
        firstName,
      });
    }
...
async function generatePrompt(session, sessionTopics, message, options = {}) {
  ...
  if (mode !== "teach") {
    ...
    // to a chatbot.
    if (options.initiativeBlock)
      systemPrompt += `\n\n${options.initiativeBlock}`;
  }

  // More than one person in this conversation? Two guardians can share a
  // session (0030_session_participant), and the model API has exactly one
  // `user` role — so without naming them, two people's turns arrive as one
  // voice and Athena attributes one guardian's words to the other.
  //
  // Worked out from the transcript rather than passed in, so a session that
  // has only ever had one speaker produces byte-identical output to before.
  const speakers = new Map();
  // Everyone present now, named from the family's own roster.
  for (const p of Array.isArray(options.participants)
    ? options.participants
    : []) {
    if (p?.profileId != null && p.name)
      speakers.set(String(p.profileId), p.name);
  }
  // Plus anyone who spoke earlier and has since left — their turns are still in
  // the transcript and still need a name in front of them.
  for (const m of Array.isArray(options.history) ? options.history : []) {
    if (
      m?.is_human &&
      m.profile_id != null &&
      m.speaker &&
      !speakers.has(String(m.profile_id))
    ) {
      speakers.set(String(m.profile_id), m.speaker);
    }
  }
  const multiParty = speakers.size > 1;
  const currentSpeakerName =
    options.speakerProfileId != null
      ? (speakers.get(String(options.speakerProfileId)) ?? null)
      : null;

  if (multiParty) {
    // She is talking to a room, so she is told it is a room. Without this she
    // sees "Sarah:" prefixes appear mid-conversation with no explanation and
    // treats them as part of what was said.
    systemPrompt +=
      `

# Who is here
` +
      `More than one person is in this conversation: ${[...speakers.values()].join(", ")}. ` +
      `Each message is prefixed with the name of the person who sent it. ` +
      `Address people by name when it is not obvious who you mean, and never ` +
      `attribute something one person said to another. They can all read the ` +
      `whole conversation, so do not repeat to one person what another just ` +
      `told you in confidence — there is no privacy between them here.`;
  }
  ...
  if (Array.isArray(options.history)) {
    for (const m of options.history) {
      ...
      if (!text) continue;
      const name =
        multiParty && m.is_human ? speakers.get(String(m.profile_id)) : null;
      contents.push({
        role: m.is_human ? "user" : "model",
        parts: [{ text: name ? `${name}: ${text}` : text }],
      });
    }
  }
  ...
  const speakerLabel =
    multiParty && currentSpeakerName ? currentSpeakerName : "User";
  contents.push({
    role: "user",
    parts: [
      {
